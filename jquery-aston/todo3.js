    ...
    input.on('keyup', function (event) {
        ...
        if (event.keyCode === 13) {
            ...
            if (text.trim()) {      // trim supprime les espaces dans une chaine de caracteres
                ...
                li += '<div class="todo-list-text single-ligne" contenteditable="true">' + task.text + '</div>';
        ...
        if (element.hasClass('todo-list-text')) {
            element.on('keyup', function (event) {
                if(event.keyCode === 13) {
                    ...
                    task.text = event.target.innerText;
                    console.log(tasks);
                }
                //console.log(event.keycode, event.target.innerText);
